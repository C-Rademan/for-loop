...
import library.*;
  ...
  // Machine opcodes
    ...
    public static final int // not all are used
      ...
      dec     =  69,
      heap    =  70,
	  sfu     = 71,							//practice for task 10
	  sfd     = 72,
	  efu    =  73,
	  efd   =   74,
	  pop3  =   75,
	  nxt   =   76,							//task 10 additions
	  popN = 	77,
	  psc  =    78,
      ...
      do {
        ...
        switch (cpu.ir) {         // execute
          ...
          case PVM.heap:          // heap dump (debugging)
            ...
            break;
		  //practice for task 10 pascal style for loop opcodes
		  case PVM.sfu:
		    if (mem[cpu.sp+1]>mem[cpu.sp]) cpu.pc = mem[cpu.pc]; //determine whether start value is greater than end value for the up forloop
			else {
				mem[mem[cpu.sp+2]] = mem[cpu.sp+1]; cpu.pc++;   //var i is assigned the start value
			}
			break;
		  case PVM.sfd:
		  ...
		  case PVM.efu:
		    if (mem[mem[cpu.sp+2]] == mem[cpu.sp]) cpu.pc++;
				...
				mem[mem[cpu.sp+2]]++; cpu.pc = mem[cpu.pc];  //go back to start of for loop
			}
			break;
		  case PVM.efd:
		    if (mem[mem[cpu.sp+2]] == mem[cpu.sp]) cpu.pc++;
			...
			break;
		  case PVM.pop3:
		    cpu.sp +=3;									//move the sp back three spaces in memory (lose the top 3 entries)
			break;
		// end of task 10 practice 
		//task10
	      case PVM.nxt:
		  // cpu.sp points at the total count of expressions
		  // after doing whatever is in the body, we need to move to the next expression's address in memory
		    if (mem[cpu.sp] == 0){
				cpu.pc++;			
			}
			else{
				mem[cpu.sp + mem[cpu.sp]+2] = mem[cpu.sp + mem[cpu.sp]-1];  //adrI should move to the address of the next expression
				mem[cpu.sp] = mem[cpu.sp]-1; //decrement the count at the top of the stack
				cpu.pc = mem[cpu.pc]; //give the program counter the destination label address (loopstart)
			}
			break;
		  case PVM.popN:
		    cpu.sp += mem[next()]; //push the stack pointer back by the count of expressions and varI and count 
			break;
		  case PVM.psc: //push the count of expressions onto the top of the stack
...
//        data.close();
    ...
    // Initializes stack machine
	  ...
	  mnemonics[PVM.nxt]      = "NXT";
	  mnemonics [PVM.popN]   =  "POPN";
	  mnemonics [PVM.psc]     ="PSC";
    } // PVM.init
