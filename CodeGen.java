...
import java.util.*;
  ...
  class CodeGen {
	...
	//practice for task 10 - pascal style for loop codegen
	public static void startForLoop (boolean up, Label destination){
		//generate prologue test
	...
	//task 10 Python for loop opcodes
	public static void next(Label destination){
		//increment address of i to move on to the next expression
		emit (PVM.nxt);
		emit (destination.address()); 
	}
	
	public static void popN(int n){
		emit (PVM.popN);
		emit (n);
	}
	
	public static void pushCount(int x){
		emit(PVM.psc);
		emit(x);
	}
